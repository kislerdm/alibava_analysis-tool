...
public:
    ...
    //cross-talk(signal retardation) noise correction coefficiencts
    float _crosstalk_c1;
    float _crosstalk_c2;
    //how many events we want to process, it's being used for scaling the hists
    int _event_num;
...
protected:
    ...
    std::vector<AlibavaCluster> findClusters(TrackerDataImpl * trkdata, AlibavaEventImpl * alibavaEvent);
