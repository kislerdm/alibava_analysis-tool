 ...
 *
 * Edited by Dmitry Kisler (dkisler@cern.ch):
 * fixed bug: sfitpoints = "fitpoints" + i, where sfitpoints is the string and i is the int variables
 *
 */
...
void EUTelDafFitter::dafEvent (LCEvent * event) {
    ...
    streamlog_out(MESSAGE4) << "dafevent" << endl;
...
void EUTelDafFitter::addToLCIO(daffitter::TrackCandidate<float,4>& track, LCCollectionVec *lcvec){
  ...
  for(size_t plane = 0; plane < _system.planes.size(); plane++){
    ...
    for(size_t mm = 0; mm < pl.meas.size(); mm++){
        cout << "at the cut = " << track.weights.at(plane)(mm) << endl;
      if( track.weights.at(plane)(mm) < 0.5f){ cout << "cut passed " << endl; continue; }
