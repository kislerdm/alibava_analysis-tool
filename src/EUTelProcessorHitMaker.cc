...
//ref hits are being claculated and written out here
void EUTelProcessorHitMaker::DumpReferenceHitDB()
{
    ...
    delete now;
    //ref hit collection is stored into the vector
...
void EUTelProcessorHitMaker::processEvent (LCEvent * event) {
    ...
    for( int iCluster = 0; iCluster < pulseCollection->getNumberOfElements(); iCluster++ )
    {
        ...
        else
        {
            ...
            //However we still do not have the sensor centre as the origin of the coordinate system.
...
void EUTelProcessorHitMaker::bookHistos(int sensorID) {
    ...
    double xMin =  -safetyFactor * (geo::gGeometry().siPlaneXSize ( sensorID )/2);
    double xMax = safetyFactor * (geo::gGeometry().siPlaneXSize ( sensorID )/2);

    double yMin = -safetyFactor * (geo::gGeometry().siPlaneYSize ( sensorID )/2);
    double yMax = safetyFactor * (geo::gGeometry().siPlaneYSize ( sensorID )/2);

    int xNBin =    safetyFactor * geo::gGeometry().siPlaneXNpixels ( sensorID );
    int yNBin =   safetyFactor * geo::gGeometry().siPlaneYNpixels ( sensorID );
    ...
    // by all its size.
    // won't be better to set the safetyFactor to 1?
    // double safetyFactor = 1.2;
    // /!hardcode D.Kisler: rebin for the CMSPixRef, ID = 8, is being set in the gearfile.
    //if(sensorID == 8) { safetyFactor = 1.; }
