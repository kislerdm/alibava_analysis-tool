...
void AlibavaCommonModeSubtraction::processEvent (LCEvent * anEvent) {
    ...
    try
    {
        ...
        for ( size_t i = 0; i < noOfChips; ++i ) //was ++i (why?)
        {
...
void AlibavaCommonModeSubtraction::fillHistos(TrackerDataImpl * trkdata){
    ...
    //which chip we are looking at
    int chipnum = getChipNum(trkdata);
    //get the noise values of this chip amoung all channels
    ...
    for ( size_t ichan = 0 ; ichan < datavec.size() ; ichan++ )
    {
        ...
        //filling the SNRvsCh histogram
        // TH2D * hSNRvsCh = dynamic_cast<TH2D*> (_rootObjectMap[getHistoNameForChip("hSNRvsCh", chipnum)]) ;
        // snr = datavec[ichan]/noiseVec[ichan];
        //  streamlog_out (MESSAGE4) << "charge = " << datavec[ichan] << endl;//" | noise = "<<noiseVec[ichan] << endl;
...
//string AlibavaCommonModeSubtraction::getHistoNameForChip(string histoName, int ichip){
//    stringstream s;
...
void AlibavaCommonModeSubtraction::bookHistos()
{
    ...
    tempHistoTitle1 << tempHistoName << ";Signal [ADC];Number of Entries";
    ...
    for (unsigned int i=0; i<chipVec.size(); i++)
    {
        ...
        for (int ichan=0; ichan<ALIBAVA::NOOFCHANNELS; ichan++)
        {
            ...
            tempHistoTitle << tempHistoName << ";Signal [ADC];Number of Entries";
