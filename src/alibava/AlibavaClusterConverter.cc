...
void AlibavaClusterConverter::processEvent (LCEvent * anEvent) {
  ...
  // HERE
 //indicates each 1000st event about its processing
  if(alibavaEvent->getEventNumber() % 1000 == 0) { streamlog_out (MESSAGE4) << "Event "<< alibavaEvent->getEventNumber() << " is being processed" <<endl; }
