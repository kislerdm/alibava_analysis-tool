...
void AlibavaTimeCutProcessor::processEvent (LCEvent * anEvent) {
    ...
    //filling the TDC distr histo
    TH1D * hTDC = dynamic_cast<TH1D*>(_rootObjectMap["hTDC"]);
    hTDC->Fill(eventTime);
...
void AlibavaTimeCutProcessor::bookHistos(){
    ...
    TH1D * _hMaskedEventsNumber = new TH1D (_hMaskedEventsNumberName.c_str(),"", 2, 0, 1000000);
    ...
    TH1D * hTDC = new TH1D ("hTDC","", 202, -1, 200);
    _rootObjectMap.insert(make_pair("hTDC", hTDC));
    hTDC->SetTitle("TDC distribution amond all events; Time [ns]; Number of events");
