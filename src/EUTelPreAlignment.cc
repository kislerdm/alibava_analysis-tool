...
void EUTelPreAlign::init () {
	...
	if( _fillHistos ) {
		...
		for(size_t i = 0; i < _sensorIDVec.size(); i++) {
            ...
            AIDA::IHistogram1D * histo1Da = AIDAProcessor::histogramFactory(this)->createHistogram1D( (basePath + tempHistoName).c_str(), 2*10*(_residualsXMax[i]-_residualsXMin[i])+1 , _residualsXMin[i], _residualsXMax[i]);
            _hitXCorr.insert( make_pair( sensorID, histo1Da) );
            histo1Da->setTitle("Residual X on the plane with respect to the fixed plane; Residual X [mm];Numb. of events");

			tempHistoName = "hitYCorr_fixed_to_" + to_string( sensorID) ;
            AIDA::IHistogram1D * histo1Db = AIDAProcessor::histogramFactory(this)->createHistogram1D( (basePath + tempHistoName).c_str(), 2*10*(_residualsYMax[i]-_residualsYMin[i])+1 , _residualsYMin[i], _residualsYMax[i]) ;
            histo1Db->setTitle("Residual Y on the plane with respect to the fixed plane; Residual Y [mm];Numb. of events");
